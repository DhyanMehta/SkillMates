...
import { useAppStore as useStoreForChats } from '@/context/AppStore';

const Requests = ({ isLoggedIn }) => {
  const navigate = useNavigate();
  const { toast } = useToast();
  const { requests, acceptRequest, rejectRequest, deleteRequest, currentUserId, users, rateRequest, getChatByRequestId } = useAppStore();
  ...
  const filteredRequests = useMemo(() => {
    ...
    return requests.filter(request => {
      const isInvolved = request.fromUserId === currentUserId || request.toUserId === currentUserId;
  ...
  }, [requests, searchQuery, statusFilter, currentUserId, users]);
  ...
  const handleAcceptRequest = async (requestId) => {
    try {
      ...
      toast({ title: 'Request accepted!', description: "Chat opened for this request." });
      // Navigate to chat thread for this request
      setTimeout(() => {
          ...
          <>
            <div className="space-y-4 mb-8">
              {paginatedRequests.map(request => {
                const isIncoming = currentUserId != null && request.toUserId === currentUserId;
                const otherUserId = currentUserId != null && request.fromUserId === currentUserId ? request.toUserId : request.fromUserId;
                const otherUser = users.find(u => u.id === otherUserId);
